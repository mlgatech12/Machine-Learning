...
public class Marketing_GA {
    ...
    public static void main(String[] args) {
        ...
        write_output_to_file("Optimization_Results", "mktg_ga_train_results.csv", header, true);

        header = "Algo" + "," + "Iterations" + "," + "Args" + ", " + "TestingAccuracy" + "," + "TestingTime" ;
        write_output_to_file("Optimization_Results", "mktg_ga_test_results.csv", header, true);
        
        int[] pop_size = {10, 25, 50, 75, 100, 150, 200, 500};
        int[] mate = {5, 15, 25, 35, 50, 75, 100, 250};
        int[] mutate = {2, 3, 5, 7, 10, 15, 20, 25};
        
        for (int iter : iters) {
        	
        	results = "";
        	    	
	        for(int k = 0; k < pop_size.length; k++) {
	            double start = System.nanoTime(), end, trainingTime, testingTime, correct = 0, incorrect = 0;
	            
	        	oa[0] = new StandardGeneticAlgorithm(pop_size[k], mate[k], mutate[k], nnop[0]);
	            ...
	            results = ""; 
	            results = oaNames[0] + "," + iter + "," + "(" +pop_size[k] + "-" +  mate[k]  + "-" +   mutate[k] + ")" + ","  + df.format(correct / (correct + incorrect) * 100)
                + "," + df.format(trainingTime) +  "," + df.format(testingTime);
	            
	            write_output_to_file("Optimization_Results", "mktg_ga_train_results.csv", results, true);
                ...
                write_output_to_file("Optimization_Results", "mktg_ga_test_results.csv", results, true);
